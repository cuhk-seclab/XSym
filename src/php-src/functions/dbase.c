...
void php3_dbase_open(INTERNAL_FUNCTION_PARAMETERS) {
	...
	if ( getParameters(ht,2,&dbf_name,&options)==FAILURE) {
		WRONG_PARAM_COUNT;
	}
...
void php3_dbase_close(INTERNAL_FUNCTION_PARAMETERS) {
	...
	if ( getParameters(ht,1,&dbh_id)==FAILURE) {
		WRONG_PARAM_COUNT;
	}
...
void php3_dbase_add_record(INTERNAL_FUNCTION_PARAMETERS) {
	...
	if ( getParameters(ht,2,&dbh_id,&fields)==FAILURE) {
		WRONG_PARAM_COUNT;
	}
	convert_to_long(dbh_id);
...
PHP_FUNCTION(dbase_replace_record) {
	...
	if ( getParameters(ht,3,&dbh_id,&fields,&recnum)==FAILURE) {
		WRONG_PARAM_COUNT;
	}
	convert_to_long(dbh_id);
...
void php3_dbase_delete_record(INTERNAL_FUNCTION_PARAMETERS) {
	...
	if ( getParameters(ht,2,&dbh_id,&record)==FAILURE) {
		WRONG_PARAM_COUNT;
	}
	convert_to_long(dbh_id);
...
void php3_dbase_create(INTERNAL_FUNCTION_PARAMETERS) {
	...
	if ( getParameters(ht,2,&filename,&fields)==FAILURE) {
		WRONG_PARAM_COUNT;
	}

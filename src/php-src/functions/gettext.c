...
void php3_textdomain(INTERNAL_FUNCTION_PARAMETERS)
{
    ...
    if ( getParameters(ht, 1, &domain) == FAILURE) {
		WRONG_PARAM_COUNT;
    }
...
void php3_gettext(INTERNAL_FUNCTION_PARAMETERS)
{
    ...
    if ( getParameters(ht, 1, &msgid) == FAILURE) {
		WRONG_PARAM_COUNT;
    }
...
void php3_dgettext(INTERNAL_FUNCTION_PARAMETERS)
{
	...
	if (getParameters(ht, 2, &domain_name, &msgid) == FAILURE)
	{
		WRONG_PARAM_COUNT;
	}
...
void php3_dcgettext(INTERNAL_FUNCTION_PARAMETERS)
{
	...
	if (getParameters(ht, 3, &domain_name, &msgid, &category) == FAILURE)
	{
		WRONG_PARAM_COUNT;
	}
	convert_to_string(domain_name);
...
void php3_bindtextdomain(INTERNAL_FUNCTION_PARAMETERS)
{
	...
	if ( getParameters(ht, 2, &domain_name, &dir) == FAILURE)
	{
		WRONG_PARAM_COUNT;
	}
	convert_to_string(domain_name);
